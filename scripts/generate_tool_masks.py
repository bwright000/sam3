...
def process_snippet(
    ...
    Args:
        ...
        prompts: List of text prompts (e.g., ["liver", "gallbladder", "tool"])
    ...
    for i, frame_path in enumerate(frame_files):
        ...
        # Run SAM3 backbone — use inference_mode (set_image is missing the
        # @torch.inference_mode decorator that set_image_batch has)
        ...
        print(f"  [{i+1}/{len(frame_files)}] {frame_name} | {elapsed:.1f}s | {mask_summary}")
...
def _draw_legend(frame, prompts, masks_dict):
    ...
    x, y = 10, 30
    for prompt in prompts:
        count = len(masks_dict.get(prompt, []))
        color = CATEGORY_COLORS.get(prompt, DEFAULT_COLOR)
